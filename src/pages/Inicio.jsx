...
const Inicio = () => {
  const { datosUsuario } = useUsuario();  // Obtén datos del usuario desde el contexto
  const navigate = useNavigate();

  const [periodos, setPeriodos] = useState([]); // Estado para almacenar la lista de períodos
  const { actualizarPeriodoSeleccionado } = usePeriodo(); // Usa el nuevo contexto

  useEffect(() => {
    if (datosUsuario && datosUsuario.uuid_usuario) {
      ...
      fetch(`https://backend-production-8aa0.up.railway.app/api/periods/periodos/${uuidUsuario}`)
        ...
        .catch(error => {
          console.error('Error al obtener los períodos desde el backend:', error);
        });
    }
  }, [datosUsuario]);

  const handleSeleccionar = (periodo) => {
    const periodoSeleccionado = periodos.find((p) => p.periodo === periodo);
                ...
                <>
                  {periodos.map((periodo, index) => (
                    <div key={index} style={{ marginBottom: "10px" }}>
                      <button
                        style={{ backgroundColor: "#00896a", marginRight: "10px" }}
                        onClick={() => handleSeleccionar(periodo.periodo)}
                      >
                        Seleccionar
                      </button>
                      <span style={{ marginRight: "10px" }}>{periodo.periodo}</span>
